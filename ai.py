...
from config import GROQ_API_KEYS, GEMINI_KEYS, MODEL, TEMPERATURE, TOP_P, MAX_TOKENS
...
def generate_from_raw_info(user_id, query, raw_info):
    ...
    try:
        ...
        response = model.generate_content(prompt, generation_config={"temperature": TEMPERATURE, "top_p": TOP_P})
        if response.candidates and response.candidates[0].content:
            print("[AI] Used Gemini for rephrase")
            return response.text.strip()
    except Exception as e:
        print(f"Gemini rephrase error: {e}, falling back to Groq")
    ...
    for _ in range(len(GROQ_API_KEYS)):
        ...
        try:
            ...
            print("[AI] Used Groq for rephrase")
            return response.choices[0].message.content.strip()
        except Exception as e:
            if "rate_limit" in str(e).lower() or "429" in str(e):
                time.sleep(1)
